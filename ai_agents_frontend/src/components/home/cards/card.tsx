...
export default function Card({
  ...
  return (
    <CardContainer className="inter-var" containerClassName="py-6">
      <CardBody className="bg-gradient-to-b from-slate-900 to-slate-950 w-xl h-auto rounded-2xl p-8 border border-slate-800 hover:border-slate-700 transition-all duration-300 relative">
          ...
          <div className="w-26 h-16 bg-slate-950/50 rounded-xl flex items-center justify-center transition-all duration-300">
            <div className="flex gap-1">
              {icons.map((item, index) => {
                ...
                return <Icon key={index} className={item.className} />;
              })}
            </div>
          </div>
        </CardItem>
        ...
        {status === "ready" ? (
          <div className="flex justify-center items-center mt-8 pt-6 border-t border-slate-800">
            <Link href={linkHref} className="flex justify-center w-full">
              <CardItem translateZ={20} as="div" className="w-xs">
